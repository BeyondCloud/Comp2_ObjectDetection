# python YOLO_small_tf_demo.py -fromfile ./000001.jpg -tofile_img ./out.jpg -tofile_txt ./output.txt
fuji_mode = 'pascal'
# fuji_mode = 'ori'
...
class YOLO_TF:
    ...
    filewrite_txt = False
    disp_console = True
    weights_file = 'weights/my_YOLO_small.ckpt'
    alpha = 0.1
    threshold = 0.2 #confidence thres
    iou_threshold = 0.5
    num_class = 20
    num_box = 2
    grid_size = 7
    classes =  ["aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa", "train","tvmonitor"]
    img_dir = '../data/VOCdevkit_test/VOC2007/JPEGImages/'
    ...
    def __init__(self,argvs = []):
        ...
        all_path = self.get_files_path()
        self.build_networks()

        if fuji_mode == 'pascal':
            self.detect_and_write(all_path)
        else:
    ...
    def build_networks(self):
        ...
        # self.saver.save(self.sess,'./weights/my_YOLO_small.ckpt')
    ...
    def detect_and_write(self,file_lst):
        with open('./test_out.txt','w') as f:
            for name in file_lst:
                print('predict ',name)
                img = cv2.imread(self.img_dir+name)
                if img is None:
                ...
                out = self.get_single_pred_slice(result)
                f.write(name)
                for box in out:
                    f.write(" %s %s %s %s %s %s" %(box[0],box[1],box[2],box[3],box[4],box[5]))
                f.write("\n")
    ...
    def show_results(self,img,results):
        ...
        for i in range(len(results)):
            ...
            w = int(results[i][3])//2
            h = int(results[i][4])//2
            if self.disp_console : print('    class : ' + str(results[i][0]) + ' , [x,y,w,h]=[' + str(x) + ',' + str(y) + ',' + str(int(results[i][3])) + ',' + str(int(results[i][4]))+'], Confidence = ' + str(results[i][5]))
            if self.filewrite_img or self.imshow:
                cv2.rectangle(img_cp,(x-w,y-h),(x+w,y+h),(0,255,0),2)
                cv2.rectangle(img_cp,(x-w,y-h-20),(x+w,y-h),(125,125,125),-1)
                cv2.putText(img_cp,self.classes[results[i][0]] + ' : %.2f' % results[i][5],(x-w+5,y-h-7),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),1)
            if self.filewrite_txt :             
                ftxt.write(str(results[i][0]) + ',' + str(x) + ',' + str(y) + ',' + str(w) + ',' + str(h)+',' + str(results[i][5]) + '\n')
        ...
        if self.filewrite_txt : 
            ...
            ftxt.close()
    def get_single_pred_slice(self,results):
        out = []
        for i in range(len(results)):
            cur_class =str(results[i][0])
            w_half = int(results[i][3])//2
            h_half = int(results[i][4])//2
            cx = int(results[i][1])
            cy = int(results[i][2])

            conf = str(np.round(results[i][5],2))
            out.append([cx-w_half,cy-h_half,cx+w_half,cy+h_half,cur_class,conf])
        return out
